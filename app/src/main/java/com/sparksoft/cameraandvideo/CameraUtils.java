...
public class CameraUtils {
    ...
    public CameraUtils(Camera camera, int cameraId) {
        mCamera = camera;
        mCameraId = cameraId;
    }

    public static CameraUtils getInstance(Camera mCamera, int mCamerId) {
        return new CameraUtils(mCamera, mCamerId);
    }
    ...
    public void initCamera(int width, int height, Activity activity) {
        ...
        Config.e("最后得到的分辨率："+"width: "+mWidth+"  height: "+mHeight);
            ...
            public void onPictureTaken(byte[] data, Camera camera) {
                ...
                try {
                    if (mPicListener != null) {
                        ...
                        Camera.CameraInfo info = new Camera.CameraInfo();
                        Camera.getCameraInfo(mCameraId, info);
                        Bitmap realBmp = FileUtil.rotaingBitmap(info.orientation, false, bitmap);

                        file = FileUtil.saveFile(realBmp, new File(mFileDir + "/", mFileName).getAbsolutePath());
                        mPicListener.onPictureTaken("", file);
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                    Config.d("错误：  " + e.getMessage());
                    if (mPicListener != null) {
